...
public class PathTest
{
    ...
    /**
     * This runs the Prim algorithm algorithms. The algorithm can either be run
     * over a random graph, or a graph specified from a file. To specify a graph
     * from a file use usage: java PathTest F [filename] [startVertex] where
     * filename is the name of the file and startVertex is the index of the
     * vertex on which to start the . To use a random graph, use java PathTest R
     * [numberOfVertices] [density] [maxweight] where number of vertices is an
     * int specifying the number of vertices, density is a double between 0 and
     * 1 specifying the the density of the graph and maxweight is an integer
     * representing the maximum possible edge weight. The random graphs will
