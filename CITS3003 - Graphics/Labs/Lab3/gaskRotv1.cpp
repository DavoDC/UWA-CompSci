...
init(void) {
    ...
    GLuint program = InitShader("shaders/vRotate1.glsl", "shaders/fshader24.glsl");
    ...
    cosAngleP = glGetUniformLocation(program, "cosAngle");
    sinAngleP = glGetUniformLocation(program, "sinAngle");
...
display(void) {
    ...
    // Adjustment local variables
    int time = glutGet(GLUT_ELAPSED_TIME);
    float angle = 0.001 * time;
    float scaleFactor = 0.693;

    // Update shader variables
    // sin tells you how much the original (x or y)
    // coordinate affects the OTHER coordinate when rotated.
    glUniform1f(sinAngleP, sin(angle) * scaleFactor);

    // cos tells you how much the original (x or y)
    // coordinate affects the SAME coordinate when rotated.
    glUniform1f(cosAngleP, cos(angle) * scaleFactor);
