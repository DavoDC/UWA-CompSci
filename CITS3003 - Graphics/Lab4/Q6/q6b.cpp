...
display(void) {
    ...
    // The rotY happens second, causing the front face to consistently
    // rotate towards the left, with the rotX adding some rotation 
    // towards the top or bottom depending on the current y-axis rotation.
    // Give multiplied matrices to shader (X FIRST THEN Y)
    glUniformMatrix3fv(rotMatrixP, 1, GL_TRUE, yRot * xRot);
