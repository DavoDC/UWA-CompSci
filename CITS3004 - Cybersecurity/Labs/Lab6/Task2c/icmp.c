...
main (int argc, char **argv)
{
  ...
  /*
	...
	1) populate ICMP data
  */
  char dataS[] = "this is the data";
  datalen = strlen(dataS);

  memcpy (data, dataS, datalen);


  
	// 2) populate IPv4 header
  // a) IPv4 header length 		(iphdr.ip_hl, use IP4_HDRLEN variable)	
  iphdr.ip_hl = IP4_HDRLEN / sizeof(uint32_t);

	// 	b) set IP version to 4		(iphdr.ip_v)
  iphdr.ip_v = 4;

	// 	c) set service type to 0	(iphdr.ip_tos)
  iphdr.ip_tos = 0;

	// 	d) set datagram length		(iphdr.ip_len, defined by IP header + ICMP header + ICMP data.
  // you need to use the htons function)
	iphdr.ip_len = htons(iphdr.ip_hl + ICMP_HDRLEN + datalen);

  // 	e) set IP sequence number 	(iphdr.ip_id, but it is unused. you need to use the htons function)
  iphdr.ip_id = htons (100);
