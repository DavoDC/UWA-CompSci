...
public class NearestNeighbour
{
    ...
    public static int[] tsp2opt(int[] cs, double[][] table)
    {
        ...
        double z = roundtrip(cs, table);
        // used to store the tours found by 2-OPT
        int[] csnew = null;
        while (csnew != cs)
        {
            csnew = cs;
            // try reversing every section of csnew
            // this code does too many iterations really - 
            // there will be functional duplicates
            for (int a = 0; a < n; a++)
                for (int b = a + 1; b < n; b++)
                {
                    // reverse csnew[a..b] and evaluate it
                    flip(csnew, a, b);
                    double zf = roundtrip(csnew, table);
                    if (zf < z)
                    {
                        cs = Arrays.copyOf(csnew, n);
                        z = zf;
                    }
                    // restore csnew[a..b]
                    flip(csnew, a, b);
