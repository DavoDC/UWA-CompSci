...
public class Bohnenspiel {
    ...
    /**
     * Sets up an arbitrary Bohnenspiel position.Checks the position for
     * plausibility and makes new copies of the arrays.
     *
     * @param b
     * @param ss
     * @param f
    ...
    public Bohnenspiel(int[] b, int[] ss, Farbe f) throws IllegalArgumentException {
        ...
        if (b.length != boardsize) {
            throw new IllegalArgumentException("Wrong number of houses: " + b.length);
        }
        board = Arrays.copyOf(b, boardsize);
        int total = 0;
        for (int k = 0; k < boardsize; k++) {
            if (board[k] < 0) {
                throw new IllegalArgumentException("Negative beans in house " + k);
            }
            total += board[k];
        }
        // validate ss
        if (ss.length != numberofstores) {
            throw new IllegalArgumentException("Wrong number of stores: " + ss.length);
        }
        stores = Arrays.copyOf(ss, numberofstores);
        for (int k = 0; k <= 1; k++) {
            if (stores[k] < 0) {
                throw new IllegalArgumentException("Negative beans in store " + k);
            }
            total += stores[k];
        }
        // validate the total number of beans
        if (total != boardsize * beansperhouse) {
            throw new IllegalArgumentException("Wrong total number of beans: " + total);
        }
        turn = f.ordinal();
        gameOver();
    }
        ...
        if (stores[turn] > boardsize * beansperhouse / 2 || stores[1 - turn] > boardsize * beansperhouse / 2) {
            ...
            for (int t = 0; t < 2 && !gameOver; t++) {
                ...
                for (int k = 0; k < numberofhouses; k++) {
                    if (board[numberofhouses * t + k] > 0) {
                        g = false;
                    }
                }
                gameOver = g;
                if (gameOver) // move the beans appropriately 
                {
                    for (int k = 0; k < numberofhouses; k++) {
    ...
    public void display(int m) {
        if (m > 0) {
            System.out.println("turn = " + (1 - turn) + ", move = " + m);
        }
        String s;
        if (turn == 0) {
            s = "             ==>";
    ...
    public void drawBoard(SimpleCanvas sc, int moved) {
        ...
        if (turn == 1) {
            sc.drawString("<==", x + store / 2, y + fontsize / 3, line);
        }
        ...
        // draw the move-marker
        if (turn == 0) {
            sc.drawString("==>", x - store * 77 / 64, y + fontsize / 3, line);
        }
    }
}
