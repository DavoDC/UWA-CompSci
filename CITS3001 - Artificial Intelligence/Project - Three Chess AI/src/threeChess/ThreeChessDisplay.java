...
public class ThreeChessDisplay extends JFrame {
    ...
    private class Square {
        ...
        /**
         * Constructs a square corresponding to the position given. This
         * calculates the cartesian coordinates of the corners of the position,
        ...
        public Square(Position pos) {
            ...
            int c = pos.getColumn();
            xs = new int[4];
            ys = new int[4]; //lower left, lower right, upper right, upper left.
        ...
        /**
         * Sets the piece of the square to the specified piece, or null if
        ...
        private Color getColour(Colour col, boolean piece, boolean coloured) {
            switch (col) {
                case RED:
                    return piece ? DARKRED : coloured ? RED : LIGHTRED;
                case GREEN:
                    return piece ? DARKGREEN : coloured ? GREEN : LIGHTGREEN;
                case BLUE:
                    return piece ? DARKBLUE : coloured ? BLUE : LIGHTBLUE;
                default:
                    return null;
            }
        }
        ...
        /**
         * Renders the square with a black border, and a light colour if parity
         * is even, and the piece in the board. The graphics object should be
         * updated after this method is called.
         */
        public void draw(Graphics g) {
            g.setColor(getColour(colour, false, coloured));
            g.fillPolygon(xs, ys, 4);
            if (board.displayLegalMoves() 
                    && manualMoveFrom != null 
                    && board.isLegalMove(manualMoveFrom, pos)) {
            ...
            if (piece != null) {
                ...
                for (int dx = -1; dx <= 1; ++dx) {
                    for (int dy = -1; dy <= 1; ++dy) {
                        if (dx != 0 || dy != 0) {
                            g.drawString(pieceStr, x + dx, y + dy);
                        }
                    }
                }
    ...
    public ThreeChessDisplay(Board board, String bluePlayer, String greenPlayer, String redPlayer) {
        ...
        for (int i = 0; i < 96; i++) {
            squares[i] = new Square(Position.values()[i]);
        }
    ...
    /**
     * @return the state of the board before move {
     * @param moveIndex}. *
     */
    ...
    /**
     * @return whether the display is currently waiting for the user to make a
     * move. *
     */
    ...
    public void drawToCanvas(Graphics2D g, Board board) {
        ...
        for (Position pos : Position.values()) {
            squares[pos.ordinal()].setPiece(board.getPiece(pos));
        }
        for (Square sq : squares) {
            sq.draw(g);
        }
    ...
    private static void drawArrow(Graphics2D g, Square from, Square to, int strokeWidth) {
        ...
        fromX += (int) (10 * Math.cos(angle));
        toX -= (int) (10 * Math.cos(angle));
        fromY += (int) (10 * Math.sin(angle));
        toY -= (int) (10 * Math.sin(angle));
    ...
    private static void setFlanks(int size) {
        ...
        flanks[0][0][0] = 7 * h_unit;
        flanks[0][0][1] = v_unit;
        flanks[0][0][2] = 8 * h_unit;
        flanks[0][0][3] = 2 * v_unit;// (x1,y1,x2,y2) coords of the left flank of the blue section of board
        flanks[0][1][0] = 5 * h_unit;
        flanks[0][1][1] = v_unit;
        flanks[0][1][2] = 5 * h_unit;
        flanks[0][1][3] = 3 * v_unit;// (x1,y1,x2,y2) coords of the middle line of the blue section of board
        flanks[0][2][0] = 3 * h_unit;
        flanks[0][2][1] = v_unit;
        flanks[0][2][2] = 2 * h_unit;
        flanks[0][2][3] = 2 * v_unit;// (x1,y1,x2,y2) coords of the right flank of the blue section of board
        flanks[1][0][0] = 7 * h_unit;
        flanks[1][0][1] = 5 * v_unit;
        flanks[1][0][2] = 5 * h_unit;
        flanks[1][0][3] = 5 * v_unit;// (x1,y1,x2,y2) coords of the left flank of the green section of board
        flanks[1][1][0] = 8 * h_unit;
        flanks[1][1][1] = 4 * v_unit;
        flanks[1][1][2] = 5 * h_unit;
        flanks[1][1][3] = 3 * v_unit;// (x1,y1,x2,y2) coords of the middle line of the green section of board
        flanks[1][2][0] = 9 * h_unit;
        flanks[1][2][1] = 3 * v_unit;
        flanks[1][2][2] = 8 * h_unit;
        flanks[1][2][3] = 2 * v_unit;// (x1,y1,x2,y2) coords of the right flank of the green section of board
        flanks[2][0][0] = h_unit;
        flanks[2][0][1] = 3 * v_unit;
        flanks[2][0][2] = 2 * h_unit;
        flanks[2][0][3] = 2 * v_unit;// (x1,y1,x2,y2) coords of the left flank of the red section of board
        flanks[2][1][0] = 2 * h_unit;
        flanks[2][1][1] = 4 * v_unit;
        flanks[2][1][2] = 5 * h_unit;
        flanks[2][1][3] = 3 * v_unit;// (x1,y1,x2,y2) coords of the middle line of the red section of board
        flanks[2][2][0] = 3 * h_unit;
        flanks[2][2][1] = 5 * v_unit;
        flanks[2][2][2] = 5 * h_unit;
        flanks[2][2][3] = 5 * v_unit;// (x1,y1,x2,y2) coords of the right flank of the red section of board
    }

    /**
     * @return {
     * @param string} if its length is below {
     * @param length}, or else its first {
