...
/**
 * An enumeration of the 96 board positions. Each position has a colour, a row
 * (0-3) and a column (0-7). *
 */
public enum Position {
    ...
    public static Position get(Colour colour, int row, int column) throws ImpossiblePositionException {
        ...
        if (index >= 0 && index < 32) {
            switch (colour) {
                ...
                case GREEN:
                    return Position.values()[index + 32];
                case RED:
                    return Position.values()[index + 64];
            }
        }
    ...
    /**
     * Gets the neighbouring cell in the given direction. Cells always move
     * forward towards the center line, so the direction is independent of piece
     * colour.
     *
     * @return the position in the specified direction.
     * @throws ImpossiblePositionException if moving backwards from the back
     * rank, or moving of the side of the board.
     *
     */
    public Position neighbour(Direction direction) throws ImpossiblePositionException {
        switch (direction) {
            ...
            case BACKWARD:
                if (row == 0) {
                    throw new ImpossiblePositionException("Moved off board");
                }
                return get(colour, row - 1, column);
            case LEFT:
                if (column == 0) {
    ...
    /**
     * returns true if the position has even parity equivalent to a black square
