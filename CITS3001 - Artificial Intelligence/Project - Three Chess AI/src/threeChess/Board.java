...
/**
 * Main class for representing game state. The board maps each position to the
 * piece at that posiiton, or null is free. It also records previous moves, as
 * well as whose move it is, and which pieces have been captured by which
 * player. *
 */
public class Board implements Cloneable, Serializable {
    ...
    /**
     * The moves taken so far, represented as an array of two positions, the
     * start and end of the move*
     */
    private ArrayList<Position[]> history;//can only be changed by taking moves
    /**
     * A map indicating which player has taken which piece, to support
     * alternative scoring methods*
     */
    private HashMap<Colour, ArrayList<Piece>> captured;
    /**
     * A Map representing the remaining time allowed for each player, in
     * milliseconds*
     */
     ...
     * Note, unlike two person chess, the Queen is always on the left, and the
     * King is always on his own colour.
     *
     * @param time the number of milliseconds each player has in total for the
     * entire game. *
     */
    public Board(int time) {
        ...
        try {
            for (Colour c : Colour.values()) {
                board.put(Position.get(c, 0, 0), new Piece(PieceType.ROOK, c));
                board.put(Position.get(c, 0, 7), new Piece(PieceType.ROOK, c));
                board.put(Position.get(c, 0, 1), new Piece(PieceType.KNIGHT, c));
                board.put(Position.get(c, 0, 6), new Piece(PieceType.KNIGHT, c));
                board.put(Position.get(c, 0, 2), new Piece(PieceType.BISHOP, c));
                board.put(Position.get(c, 0, 5), new Piece(PieceType.BISHOP, c));
                board.put(Position.get(c, 0, 3), new Piece(PieceType.QUEEN, c));
                board.put(Position.get(c, 0, 4), new Piece(PieceType.KING, c));
                for (int i = 0; i < 8; i++) {
                    board.put(Position.get(c, 1, i), new Piece(PieceType.PAWN, c));
                }
            }
        } catch (ImpossiblePositionException e) {
        }//no impossible positions in this code
    ...
    /**
     * @return whether in manual mode, the legal moves should be displayed on
     * the board. *
     */
    ...
    /**
     * Return a set of all the positions of pieces belonging to a player. This
     * is a method of convenience, but is not very fast. Time concious players
     * may prefer to maintain their own data structure that persists between
     * moves.
     *
     * @param player the Colour of the player owing the pieces
     * @return a Set of the positions that are occupied by a piece of the given
     * colour. *
     */
    ...
    /**
     * @return a set of all the pieces captured by {
     * @param player}. *
     */
    ...
    /**
     * Performs one step of a move such as the L shaped move of a knight, or a
     * diagonal step of a Bishop. Rooks, Bishops and Queens may iterate one step
     * repeatedly, but all other pieces can only move one step per move. Note
     * the colour of the piece is relevant as moving forward past the 4th row is
     * actually moving backwards relative to the board. It does not check
     * whether the move is legal or possible.
     *
     ...
     * @throws ImpossiblePositionException if the step takes piece off the
     * board. *
     */
     ...
     * the colour of the piece is relevant as moving forward past the 4th row is
     * actually moving backwards relative to the board. It does not check
     * whether the move is legal or possible. Overloaded operation with an extra
     * parameter to allow for checking if an iterated move needs to be reversed.
     *
     * @param piece the piece being moved
     * @param step an array of the direction sequence in the step
     * @param current the starting position of the step.
     * @param reverse whether the steps out to be reversed (if the piece crosses
     * board section).
     * @return the position at the end of the step.
     * @throws ImpossiblePositionException if the step takes piece off the
     * board. *
    ...
    /**
     * Checks if a move is legal. The move is specified by the start position
     * (where the moving piece begins), and the end position, where the piece
     * intends to move to. The conditions checked are: there is a piece at the
     * start position; the colour of that piece correspond to the player whose
     * turn it is; if there is a piece at the end position, it cannot be the
     * same as the moving piece; the moving piece must be executing one or more
     * steps allowed for their type, including two steps forward for initial
     * pawn moves and castling left and right; pieces that can make iterated
     * moves must iterate a single step type and cannot pass through any other
     * piece. Note, en passant is not allowed, you can castle after King or rook
     * have moved but they must have returned to their initial position, all
     * pawns reaching the back row are promoted to Queen, you may move into
     * check, and you may leave your king in check, and you may castle across
     * check.
     *
    ...
    /**
     * Executes a legal move. If a piece is taken it is replaced at that
     * position by the taking piece. The game ends when a King is taken. When a
     * pawn reaches the back rank it is automatically promoted to Queen.
     *
    ...
    public void move(Position start, Position end, int time) throws ImpossiblePositionException {
        if (isLegalMove(start, end)) {
            ...
            if (timeLeft.get(mover.getColour()) < 0) {
                gameOver = true;
            } else {
            ...
            throw new ImpossiblePositionException("Illegal Move: " + start + "-" + end);
        }
    }
     ...
     * Executes a legal move. If a piece is taken it is replaced at that
     * position by the taking piece. The game ends when a King is taken. When a
     * pawn reaches the back rank it is automatically promoted to Queen. Method
     * overloaded to allow for untimed games.
     *
     * @param start the starting position of the move
     * @param end the ending position of the move
     * @throws ImpossiblePositionException if the move is not legal *
     ...
     * @return an array containing the start position and the end position of
     * the move, in that order
     * @throws ArrayIndexOutOfBoundsException if the index does not correspond
     * to a move. *
     */
    ...
    /**
     * Calculates a players score, used for some variants of the game. The score
     * is the combined piece values of the players pieces on the board, plus the
     * value of the pieces taken by that player. This is a convenience method
     * which gives a basic utility value. It can be used to encourage more
     * aggressive play in agents, but the traditional scoring is +1 for taking a
     * King, and -1 for losing a King,
     *
    ...
    public int score(Colour player) {
        ...
        for (Piece piece : captured.get(player)) {
            score += piece.getValue();
        }
        return score;
    }
    ...
    /**
     * The winner of the game is the player who takes another player's King, or
     * the player with the highest score when another player runs out of time.
     *
    ...
    /**
     * The loser of the game is the player who had their King taken, or the
    ...
    /**
     * Returns a deep clone of the board state, such that no operations will
     * affect the original board instance.
     *
